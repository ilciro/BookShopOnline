...
public class CsvUtente implements UserInterface{
    ...
    @Override
    public User getUserList(File fd, int idUser, String mail, int id,String pwd) throws CsvValidationException, IOException {
        return getUserData(fd,idUser,mail,id,pwd);
    }
    ...
    private static synchronized boolean insertUser(User u) throws IOException, CsvValidationException {
        ...
        gVector[GETINDEXIDUSER]= String.valueOf(getIdMax());
    ...
    private static synchronized User getUserData(File fd,int id,String mail,int id1,String pass) throws IOException, CsvValidationException {
        CSVReader reader=new CSVReader(new BufferedReader(new FileReader(fd)));
        String []gVector;
        while ((gVector = reader.readNext()) != null) {
            boolean recordFound =  (gVector[GETINDEXEMAIL].equals(mail)||   gVector[GETINDEXPWD].equals(pass))  ;
            if(recordFound)
            {
