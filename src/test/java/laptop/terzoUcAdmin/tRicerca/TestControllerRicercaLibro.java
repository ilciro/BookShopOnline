...
class TestControllerRicercaLibro {
    ...
    @ParameterizedTest
    @ValueSource(strings = {"Erasgon Vol 1","Rizzoli","Christopher Paolini"})
    void testRicercaLibroDb(String strings) throws CsvValidationException, IOException, IdException {
        vis.setTypeAsBook();
        vis.setTypeOfDb("db");
        assertNotEquals(0,cRic.getIdOggetto(strings));
    }
    @ParameterizedTest
    @ValueSource(strings = {"Sulla verità","Laterza","Donand Davidson"})
